...
class MigrateTenantCommand extends Command
{
    ...
    public function handle(TenantManager $tenants): int
    {
        ...
        try {
            $migrationPath = OrchestraPath::migrations();

            if ($this->option('all')) {
                foreach (Oor::getTenants() as $t) {
                    $tenants->runFor($t, function (TenantContext $tenant) use (&$migratedTenants, $migrationPath) {
                ...
                if ($name = $this->argument('name')) {
                    $tenants->runFor($name, function (TenantContext $tenant) use (&$migratedTenants, $migrationPath) {
                        runInConsole(fn () => $this->info("Migration du tenant: {$tenant->name}"));
                        Artisan::call('migrate', ['--path' => $migrationPath, '--force' => true]);
                        runInConsole(fn () => $this->line(Artisan::output()));
            ...
            if ($rollbackOnFail && (\count($migratedTenants) > 0)) {
                ...
                foreach ($migratedTenants as $tenantName) {
                    $tenants->runFor($tenantName, function (TenantContext $tenant) use ($migrationPath) {
